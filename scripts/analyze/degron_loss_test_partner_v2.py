...
def statistical_test(tmp, eoi, other, num_sim=10000):
    feat_col_name = "score sum (loss) {}".format(other)
...
def concat_results(r1, r2):
    ...
    result_loss = pd.concat([df1, r1.loc[only_r1], r2.loc[only_r2]])
...
def main(opts):
    ...
    if opts["domain"]:
        myflag = is_inframe & is_domain_retained
    else:
        myflag = is_inframe
    ...
    tmp.loc[~is_inframe, 'score sum (loss) {}'.format(other)] = 0
    ...
    result_loss = concat_results(result1, result2)
